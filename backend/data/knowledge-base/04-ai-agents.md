...
An AI agent is an LLM wrapped in a loop that lets it take actions, such as
calling functions, querying databases and hitting APIs, rather than only
producing text. The most common pattern is the **ReAct loop** (Reason + Act):
...
AutoGen provide higher-level abstractions, such as memory, multi-agent
orchestration and graph-based control flow, on top of this same fundamental
tool-calling loop.
